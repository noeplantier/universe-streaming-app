/**
 * app/(tabs)/reels/index.tsx — UNIVERSE · v8
 *
 * ★ Consomme useReelsUI() — même Animated.Value que la NavBar dans _layout
 * ★ TopHeader disparaît/réapparaît en même temps que la NavBar
 * ★ useFocusEffect : restoreNavBar() quand on quitte l'onglet
 */
...
function mapReel(r:SupabaseReel): FeedFilm {
  return {
    id:r.id, video_url:resolveVideoUrl(r.video_url),
    poster_url:`https://picsum.photos/seed/${r.id}/720/1280`,
    title:r.title??'', genre:r.genre??'', director:r.director??'',
    year:r.year??'', synopsis:r.synopsis??'',
    duration:Number(r.duration??0),
    likes_count:r.likes_count??0, views_count:r.views_count??0,
    created_at:r.created_at, is_liked:false, is_saved:false,
    tags:r.genre?[`#${r.genre}`]:[],
  };
}

const COLS = 'id,created_at,user_id,video_url,title,genre,director,year,synopsis,duration,likes_count,views_count,status';

async function fetchApprovedReels(): Promise<SupabaseReel[]> {
  const { count, error:cErr } = await supabase
    .from('reels').select('id',{count:'exact',head:true}).eq('status','approved');
  if (cErr) {
    const { data } = await supabase.from('reels').select(COLS)
      .eq('status','approved').order('created_at',{ascending:false}).limit(100);
  ...
  const pages = await Promise.all(
    Array.from({length:Math.max(1,Math.ceil(total/100))},(_,i) =>
      supabase.from('reels').select(COLS).eq('status','approved')
        .order('created_at',{ascending:false}).range(i*100,i*100+99)
        .then(({data,error})=>error?[]:(data??[]) as SupabaseReel[]),
    ),
  );
  return pages.flat();
}
...
function useReelsFeed(feedKey:MenuKey) {
  ...
  useEffect(()=>{
    const ch=supabase.channel(`reels_rt_${Date.now()}`)
      .on('postgres_changes',{event:'UPDATE',schema:'public',table:'reels'},({new:row})=>{
        const r=row as SupabaseReel;
        if(r.status==='approved'){
          setFilms(prev=>prev.some(x=>x.id===r.id)?prev.map(x=>x.id===r.id?mapReel(r):x):[mapReel(r),...prev]);
        } else {
  ...
  const toggleLike=useCallback((id:string)=>{
    setFilms(prev=>prev.map(f=>{
      ...
      supabase.from('reels').update({likes_count:f.likes_count+(liked?1:-1)}).eq('id',id).then(()=>{});
      return {...f,is_liked:liked,likes_count:f.likes_count+(liked?1:-1)};
    }));
  },[]);

  const incrementViews=useCallback((id:string,current:number)=>{
    supabase.from('reels').update({views_count:current+1}).eq('id',id).then(()=>{});
  },[]);
...
export default function ReelsScreen() {
  ...
  // ★ Contexte — navBarOpacity pilote NavBar + TopHeader ensemble
  const { navBarOpacity, uiVisible, setUIVisible, restoreNavBar } = useReelsUI();
  ...
  const commitIndex = useCallback((next:number)=>{
    ...
    setUIVisible(true); // Changer de vidéo → restore le chrome
  ...
  const onScrollEndDrag=useCallback((e:any)=>{
    ...
    snapTimer.current=setTimeout(()=>commitRef.current(Math.round(y/ITEM_H)),80);
  },[ITEM_H]);
  ...
  useEffect(()=>{
    ...
    InteractionManager.runAfterInteractions(()=>{ flatRef.current?.scrollToIndex({index:i,animated:true}); });
  },[newReelId,films]);
  useEffect(()=>{ scrolledNew.current=false; },[newReelId]);

  // ★ Focus : restore à l'arrivée, restore + clean à la sortie
  useFocusEffect(useCallback(()=>{
    ...
    return ()=>{
      ...
      // ★ CRITIQUE : on quitte l'écran → restore la NavBar pour les autres écrans
      restoreNavBar();
    };
  },[setUIVisible,restoreNavBar]));
  ...
  const getItemLayout=useCallback((_:unknown,i:number)=>({length:ITEM_H,offset:ITEM_H*i,index:i}),[ITEM_H]);
  ...
  const onScrollToIndexFailed=useCallback(({index}:{index:number})=>{
    ...
    InteractionManager.runAfterInteractions(()=>{ flatRef.current?.scrollToIndex({index,animated:true}); });
  },[ITEM_H]);
  ...
  return (
    <View style={s.root}>
      ...
      <FlatList
        ...
        windowSize={9} maxToRenderPerBatch={3}
        initialNumToRender={3} updateCellsBatchingPeriod={50}
        removeClippedSubviews={false} overScrollMode="never"
        bounces={false} showsVerticalScrollIndicator={false}
        directionalLockEnabled
      />

      {/* ★ TopHeader — même navBarOpacity que la CustomNavBar */}
      <Animated.View
        style={[s.header,{opacity:navBarOpacity}]}
