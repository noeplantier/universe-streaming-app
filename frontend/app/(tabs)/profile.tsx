...
export default function ProfileScreen() {
  ...
  const renderCreations=()=>{
    ...
    return(<View><View style={{flexDirection:'row',paddingHorizontal:H_PAD,paddingVertical:14,marginBottom:4}}>{[{icon:'film-outline' as const,v:`${reels.length}`,l:'vidéos'},{icon:'time-outline' as const,v:fmt(reels.reduce((a,b)=>(a+(b.duration??0)),0),true),l:'durée totale'}].map((s,i)=><View key={i} style={{flexDirection:'row',alignItems:'center',gap:6,marginRight:18}}><Ionicons name={s.icon} size={10} color={C.muted}/><Text style={{color:C.muted,fontSize:9,fontWeight:'600'}}>{s.v} {s.l}</Text></View>)}</View><HScrollRow>{reels.map(r=><ReelCard key={r.id} reel={r} isHot={r.id===hotId}/>)}</HScrollRow><View style={{height:110}}/></View>);
  };
