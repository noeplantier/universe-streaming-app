/**
 * app/profile.tsx  —  VERSION PRODUCTION-SAFE
 *
 * FIXES vs version précédente :
 *  1. Auth via getSession() + onAuthStateChange  → fonctionne même si la
 *     session se restaure après le premier mount (cas fréquent en prod)
 *  2. userId initialisé dès que possible via useAuth() ET Supabase
 *  3. loadData() accepte un uid explicite → plus de dépendance au state async
 *  4. Chaque fetch est wrappé try/catch avec log structuré
 *  5. fetchWithRetry() : 2 tentatives sur les erreurs réseau transitoires
 *  6. Timeouts sur les requêtes Supabase (AbortController)
 *  7. Vérification env vars au boot → alerte visible en dev
 *  8. Fallback data conservé si un fetch échoue (pas d'écran blanc)
 */
...
// ─────────────────────────────────────────────────────────────────────────────
// ★ ENV CHECK — affiche un warning visible en dev si Supabase n'est pas conf.
// ─────────────────────────────────────────────────────────────────────────────
if (__DEV__) {
  ...
  if (!url || !key) {
    console.error(
      '[profile] ⚠️  EXPO_PUBLIC_SUPABASE_URL ou EXPO_PUBLIC_SUPABASE_ANON_KEY manquant.\n' +
      'En production : vérifiez vos variables d\'environnement dans le dashboard EAS / Vercel / etc.'
    );
  }
}
...
// ─────────────────────────────────────────────────────────────────────────────
// ★ PRODUCTION-SAFE AUTH HELPER
// Retourne l'uid depuis la session locale (pas de round-trip réseau).
// getSession() lit depuis AsyncStorage/SecureStore → fonctionne hors-ligne
// et immédiatement au mount, contrairement à getUser() qui frappe l'API.
// ─────────────────────────────────────────────────────────────────────────────
async function getAuthUserId(): Promise<string | null> {
  try {
    // 1. Essai rapide depuis la session locale (pas de requête réseau)
    const { data: { session }, error: sessErr } = await supabase.auth.getSession();
    if (!sessErr && session?.user?.id) return session.user.id;

    // 2. Fallback : refresh via réseau (en cas de token expiré)
    const { data: { user }, error: userErr } = await supabase.auth.getUser();
    if (!userErr && user?.id) return user.id;

    console.warn('[profile] Auth: aucun utilisateur trouvé', { sessErr, userErr });
    return null;
  } catch (e) {
    console.error('[profile] Auth exception:', e);
    return null;
  }
}

// ─────────────────────────────────────────────────────────────────────────────
// ★ FETCH WITH RETRY — 2 tentatives, délai exponentiel
// ─────────────────────────────────────────────────────────────────────────────
async function fetchWithRetry<T>(
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      if (error) {
        console.warn(`[profile] ${label} tentative ${attempt}/${retries}:`, error?.message ?? error);
        if (attempt < retries) await new Promise(r => setTimeout(r, 600 * attempt));
        continue;
      }
      return data;
    } catch (e) {
      console.error(`[profile] ${label} exception tentative ${attempt}:`, e);
      if (attempt < retries) await new Promise(r => setTimeout(r, 600 * attempt));
    }
  }
  console.error(`[profile] ${label} → échec après ${retries} tentatives`);
  return null;
}
...
export default function ProfileScreen() {
  ...
  // ★ userId géré dans un ref ET dans un state pour éviter les races conditions
  ...
  // Refs stables pour les closures realtime
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // ★ AUTH INITIALIZATION — production-safe
  // Stratégie : getSession() (local, immédiat) + listener onAuthStateChange
  // pour rattraper le cas où la session se restaure après le mount.
  // ─────────────────────────────────────────────────────────────────────────
  useEffect(() => {
    let mounted = true;

    // Tentative 1 : session locale (rapide, sans réseau)
    getAuthUserId().then(uid => {
      if (uid && mounted) setUserIdSafe(uid);
    });

    // Tentative 2 : si l'uid vient du contexte AuthContext
    if (user?.id) setUserIdSafe(user.id);

    // Listener : gère le cas prod où la session arrive après le premier render
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // FETCH FUNCTIONS — toutes via fetchWithRetry
  // ─────────────────────────────────────────────────────────────────────────
  ...
  const loadReviews = useCallback(async (uid: string) => {
    ...
    setReviews((data as any[]).map((c: any) => ({
      ...
      film: {
        ...
        title: String(c.film_title ?? c.title ?? '—'),
  ...
  const loadTaggedWorks = useCallback(async (uid: string, name: string, username: string) => {
    ...
    try {
      ...
      const results = await Promise.allSettled([...queries, dirQuery]);
      const all = results.flatMap(r =>
        r.status === 'fulfilled' ? ((r.value.data ?? []) as Work[]) : []
      );
      const unique = [...new Map(all.map(w => [w.id, w])).values()];
      setTaggedWorks(unique);
    } catch (e) {
      console.warn('[profile] loadTaggedWorks:', e);
    }
  }, []);

  // ─────────────────────────────────────────────────────────────────────────
  // ★ LOAD DATA — accepte un uid explicite pour éviter la dépendance au state
  // ─────────────────────────────────────────────────────────────────────────
  const loadData = useCallback(async (uidOverride?: string) => {
    // Priorise : override > ref (synchrone) > state (async) > auth
    ...
    if (!uid) {
      console.warn('[profile] loadData: uid introuvable, abandon');
      setLoading(false);
      setRefreshing(false);
      return;
    }

    // Met à jour userId si on l'a trouvé via getAuthUserId
    if (uid !== userIdRef.current) setUserIdSafe(uid);

    setLoading(true);
    try {
      // Profil en premier (pour avoir display_name avant les tags)
      ...
      // Recommandations basées sur favs + historique
      fetchRecommendations(favs, watched).catch(e =>
        console.warn('[profile] fetchRecommendations:', e)
      );
    } catch (e) {
      console.error('[profile] loadData exception:', e);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  }, [
    userId, setUserIdSafe,
    loadProfileData, fetchFavWorks, fetchWatchedWorks,
    loadReviews, loadUserReels, fetchRecommendations,
  ]);

  // ─────────────────────────────────────────────────────────────────────────
  // ★ DÉCLENCHER loadData dès que userId est disponible
  // ─────────────────────────────────────────────────────────────────────────
  useEffect(() => {
    if (userId) loadData(userId);
  }, [userId]); // eslint-disable-line react-hooks/exhaustive-deps
  // Note: on dépend volontairement que de userId (pas loadData)
  // pour éviter les boucles infinies. loadData est stable grâce à useCallback.

  // Tags : recharge quand le profil (display_name) est chargé
  useEffect(() => {
  ...
  // ★ useFocusEffect — recharge à chaque retour sur l'écran (ex: depuis edit.tsx)
  ...
  useEffect(() => {
    ...
    const chProf = supabase.channel(`pf_profile_${ts}`)
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'profiles' },
        ({ new: row }) => {
          if ((row as any).id === uid) setProfileData(prev => ({ ...prev, ...(row as any) }));
        })
      .subscribe(status => {
        if (status === 'CHANNEL_ERROR') console.warn('[profile] RT profile error');
      });

    const chFav = supabase.channel(`pf_fav_${ts}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'user_favorites' },
        () => fetchFavWorks(uid).then(favs => fetchRecommendations(favs, watchedRef.current)))
      .subscribe();

    const chHist = supabase.channel(`pf_hist_${ts}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'user_history' },
        () => fetchWatchedWorks(uid).then(watched => fetchRecommendations(favRef.current, watched)))
      .subscribe();
    ...
    const chReels = supabase.channel(`pf_reels_${ts}`)
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'reels' },
        ({ new: row }) => {
          ...
          if (r.id) setUserReels(prev => prev.some(x => x.id === r.id) ? prev : [r, ...prev]);
        })
    ...
    return (
      <View>
        <SectionHeader icon="trophy" label="Œuvres favorites" subtitle="Tes œuvres préférées"
          count={favWorks.length} accentColor="#fff" onViewAll={()=>router.push('/profile/favorites' as any)}/>
        {favWorks.length===0
    ...
    if (userReels.length===0) return (
      <View style={{paddingTop:60,paddingHorizontal:H_PADDING}}>
        <EmptyState icon="videocam-outline" text="Aucune création"
          subtext="Importez vos vidéos depuis l'onglet Créer"/>
  ...
  const renderTags = () => {
    ...
    const asDirector = taggedWorks.filter(w =>
      w.director && [displayName, profileData.username].some(n =>
        n && w.director?.toLowerCase().includes(n.toLowerCase())
      )
    );
    ...
    return (
      <View>
        ...
        {asDirector.length > 0 && (
          <>
            <SectionHeader icon="film-outline" label="En tant que réalisateur·rice"
              subtitle="Œuvres que vous avez dirigées" accentColor={C.gold}/>
        ...
        {asCast.length > 0 && (
          <>
            <SectionHeader icon="people-outline" label="Au générique"
              subtitle="Œuvres où vous êtes crédité·e" accentColor={G.primary}/>
    ...
    <View style={pg.root}>
      ...
      >
        <SafeAreaView edges={['top']}>
          <LinearGradient
            colors={['rgba(13,13,18,0.60)', 'transparent']}
            style={pg.topGrad}
            pointerEvents="none"
          />
          ...
          {tabs.map(({ icon, label }, idx) => {
            ...
            return (
              ...
              >
                ...
                {pendingBadge > 0 && (
                  <View style={pg.badge}>
                    <Text style={pg.badgeTxt}>{pendingBadge}</Text>
                  </View>
                )}
              </TouchableOpacity>
            );
...
const pg = StyleSheet.create({
  ...
  tabIndicator:{ position: 'absolute', top: 0, left: '20%', right: '20%', height: 2, backgroundColor: G.primary, borderBottomLeftRadius: 2, borderBottomRightRadius: 2 },
