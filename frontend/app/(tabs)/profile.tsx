 ...
 * Profil Instagram-style — Universe Cinéma Indépendant
 *
 * FIXES déploiement (sans changer l'UX/UI) :
 *  ✓ BlurView → require conditionnel + fallback View
 *  ✓ import api → supprimé (non utilisé)
 *  ✓ import NotifService → require conditionnel (peut ne pas exister)
 *  ✓ import { StatColumn } → retiré (non utilisé dans ce fichier)
 *  ✓ DEFAULT_SEEN / FilmItem → retirés (non utilisés)
 *  ✓ G.amber → '#F59E0B'  /  G.cyan → '#14B8A6'  (inline, G.amber/cyan undefined)
 *  ✓ cric.border → position:'absolute' au lieu de spread StyleSheet.absoluteFillObject
 *  ✓ BlurView dans ProfileHeader/TopNav → SafeBlur (View sur web, BlurView sur natif)
 *  ✓ user.films_seen_count → (user as any).films_seen_count ?? 0
 *  ✓ Shimmer → animation partagée module-level (une seule Animated.Value)
 *  ✓ Chargement : profil header en priorité, sections en parallèle
 */
...
// ── expo-video-thumbnails : web-safe ─────────────────────────────────────────
let VideoThumbnails: any = null;
if (Platform.OS !== 'web') {
  try { VideoThumbnails = require('expo-video-thumbnails'); } catch {}
}

// ── BlurView : web-safe + fallback View ──────────────────────────────────────
let _BlurView: any = null;
if (Platform.OS !== 'web') {
  try { _BlurView = require('expo-blur').BlurView; } catch {}
}
function SafeBlur({
  intensity = 12, style, children,
}: { intensity?: number; style?: any; children?: React.ReactNode }) {
  if (_BlurView) {
    return <_BlurView intensity={intensity} tint="dark" style={style}>{children}</_BlurView>;
  }
  return <View style={[style, { backgroundColor: 'rgba(13,34,64,0.70)' }]}>{children}</View>;
}

// ── NotifService : conditionnel (peut ne pas exister) ────────────────────────
let NotifService: any = null;
try { NotifService = require('@/services/notifService').default; } catch {}

const LOGO = require('@/assets/images/logouniverse2.png');

// ─────────────────────────────────────────────────────────────────────────────
// TOKENS — valeurs inline pour éviter undefined (G.amber / G.cyan non définis)
// ─────────────────────────────────────────────────────────────────────────────
const C = {
  bg:       '#03000A',
  ...
  primary:  G.primary,
  success:  '#22C55E',
  amber:    '#F59E0B',   // ★ inline — G.amber est undefined
  cyan:     '#14B8A6',   // ★ inline — G.cyan est undefined
  error:    '#EF4444',
} as const;
...
function useVideoThumbnail(videoUrl: string, thumbnailUrl: string|null): string|null {
  ...
  useEffect(() => {
    ...
    if (!videoUrl || !VideoThumbnails || Platform.OS === 'web') return;
    let active = true;
...
// ─────────────────────────────────────────────────────────────────────────────
// SHIMMER — animation partagée (une seule Animated.Value module-level)
// ─────────────────────────────────────────────────────────────────────────────
const _shimOp = new Animated.Value(0.22);
let   _shimStarted = false;
function ensureShimmer() {
  if (_shimStarted) return;
  _shimStarted = true;
  Animated.loop(Animated.sequence([
    Animated.timing(_shimOp, { toValue:0.50, duration:850, useNativeDriver:true }),
    Animated.timing(_shimOp, { toValue:0.22, duration:850, useNativeDriver:true }),
  ])).start();
}
...
const Shimmer = memo(function Shimmer({ w, h, r = 8 }: { w: number|string; h: number; r?: number }) {
  ...
  return (
    <Animated.View style={{
      ...
      backgroundColor:'rgba(255,255,255,0.09)', opacity:_shimOp,
    }}/>
  );
});

// ★ G.amber / G.cyan → valeurs inline C.amber / C.cyan
...
const WorkTagCard = memo(function WorkTagCard({ work, userLabel }: { work:Work; userLabel:string }) {
  ...
  const creditLabel = useMemo(() => {
    if (work.director && work.director.toLowerCase().includes(userLabel.toLowerCase())) return 'Réalisateur·rice';
  ...
  return (
    <TouchableOpacity style={{ marginRight:12 }} onPress={() => router.push(`/film/${work.id}` as any)} activeOpacity={0.88}>
      <View style={wtc.card}>
        ...
        {work.is_original && <View style={wtc.origBadge}><Text style={wtc.origTxt}>ORIGINAL</Text></View>}
  ...
  return (
    <TouchableOpacity style={{ marginRight:12 }} onPress={onPress} activeOpacity={0.88}>
      <View style={cric.card}>
        ...
        {/* ★ FIX : position:'absolute' au lieu de spread StyleSheet.absoluteFillObject */}
...
const cric = StyleSheet.create({
  ...
  // ★ FIX : pas de spread StyleSheet.absoluteFillObject dans StyleSheet.create
  border:   { position:'absolute', top:0, left:0, right:0, bottom:0, borderRadius:16, borderWidth:1, borderColor:'rgba(124,94,252,0.18)' },
});

// ─────────────────────────────────────────────────────────────────────────────
// PROFILE HEADER — Instagram-style (UX/UI identique, BlurView web-safe)
// ─────────────────────────────────────────────────────────────────────────────
    ...
    <View style={hdr.wrap}>
      <View style={hdr.topRow}>
        ...
        <View style={hdr.statsRow}>
          {[
            { v: fmtNumber(filmCount),      l: 'films'     },
            { v: fmtNumber(critiqueCount),  l: 'critiques' },
            { v: fmtNumber(reelCount),      l: 'créas' },
          ].map(({ v, l }, i, arr) => (
            <React.Fragment key={l}>
              <View style={hdr.statItem}>
                <Text style={hdr.statVal}>{v}</Text>
                <Text style={hdr.statLabel}>{l}</Text>
              </View>
      ...
      {/* ★ SafeBlur au lieu de BlurView direct */}
        ...
        <View style={hdr.socialRow}>
          {socialLinks.map(l => (
            <TouchableOpacity key={l.key} style={hdr.socialBtn}
              ...
              <SafeBlur intensity={Platform.OS==='ios'?16:10} style={StyleSheet.absoluteFillObject}/>
...
// ─────────────────────────────────────────────────────────────────────────────
// TOP NAV (SafeBlur au lieu de BlurView direct)
// ─────────────────────────────────────────────────────────────────────────────
      ...
      <View style={nav.right}>
        {[
          ...
          <TouchableOpacity key={icon} style={nav.iconBtn} onPress={cb} activeOpacity={0.75}>
            <SafeBlur intensity={Platform.OS==='ios'?14:9} style={nav.iconBg}/>
...
export default function ProfileScreen() {
  ...
  const loadTaggedWorks = useCallback(async (uid: string, name: string, username: string) => {
    ...
    try {
      const queries = searchNames.map(n =>
        supabase.from('works').select('id,title,category,genre,year,likes,comments,image,is_original,adjective,duration,description,director,cast_list').overlaps('cast_list', [n])
      );
  ...
  const loadData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      await loadProfileData(uid); // header d'abord
  ...
  }, [userId, loadProfileData, fetchFavWorks, fetchWatchedWorks, loadReviews, loadUserReels, fetchRecommendations]);
  ...
  useEffect(() => {
    ...
    const ts = Date.now();
    const channels = [
      supabase.channel(`pf_prof_${ts}`).on('postgres_changes', {event:'UPDATE',schema:'public',table:'profiles'},
        ({new:row}) => { if ((row as any).id === userId) setProfileData(p => ({...p,...(row as any)})); }).subscribe(),
      supabase.channel(`pf_fav_${ts}`).on('postgres_changes', {event:'*',schema:'public',table:'user_favorites'},
        () => fetchFavWorks(userId).then(f => fetchRecommendations(f, watchedRef.current))).subscribe(),
      supabase.channel(`pf_hist_${ts}`).on('postgres_changes', {event:'*',schema:'public',table:'user_history'},
        () => fetchWatchedWorks(userId).then(w => fetchRecommendations(favRef.current, w))).subscribe(),
      supabase.channel(`pf_crit_${ts}`).on('postgres_changes', {event:'INSERT',schema:'public',table:'critiques'},
        () => loadReviews(userId)).subscribe(),
      supabase.channel(`pf_reels_${ts}`)
        .on('postgres_changes', {event:'INSERT',schema:'public',table:'reels'},
          ({new:row}) => { const r=row as UserReel; if(r.id) setUserReels(p=>p.some(x=>x.id===r.id)?p:[r,...p]); })
        .on('postgres_changes', {event:'UPDATE',schema:'public',table:'reels'},
          ({new:row}) => { const r=row as UserReel; setUserReels(p=>p.map(x=>x.id===r.id?r:x)); })
        .subscribe(),
    ];
  ...
  const displayName = profileData.display_name || (user as any)?.username || 'Cinéaste';
  const avatarUri   = profileData.avatar_url   || (user as any)?.avatar_url || `https://i.pravatar.cc/150?u=${(user as any)?.id}`;
  ...
  const renderFilms = () => {
    if (loading) return (
      <View>
        <SkeletonSection accentColor={C.gold}/>
        <SkeletonSection accentColor={C.amber}/>   {/* ★ FIX: C.amber au lieu de G.amber */}
        <SkeletonSection accentColor={C.cyan}/>    {/* ★ FIX: C.cyan au lieu de G.cyan  */}
    ...
    return (
      <View>
        ...
        {favWorks.length===0 ? <EmptyState icon="heart-outline" text="Aucun favori" subtext="Sauvegarde tes films avec l'étoile"/> : <HScrollRow>{favWorks.map((f,i) => <PortraitCard key={`fav-${f.id}`} item={f} rank={i+1}/>)}</HScrollRow>}
        <View style={pg.div}/>
        ...
        {sortedReviews.length===0 ? <EmptyState icon="chatbubble-outline" text="Aucune critique publiée"/> : <HScrollRow>{sortedReviews.map((r,i) => <CritiqueCard key={r.id} review={r} rank={i+1} onPress={() => router.push(`/review/${r.id}` as any)}/>)}</HScrollRow>}
        <View style={pg.div}/>
        ...
        {watchedWorks.length===0 ? <EmptyState icon="film-outline" text="Aucun visionnage" subtext="Marque des films comme vus"/> : <HScrollRow>{watchedWorks.map((f,i) => <PortraitCard key={`seen-${f.id}`} item={f} rank={i+1}/>)}</HScrollRow>}
        <View style={pg.div}/>

        <SectionHeader icon="sparkles" label="Recommandés" subtitle="Basé sur vos goûts" accentColor="#fff"/>
        {recommendations.length===0 ? <EmptyState icon="planet-outline" text="Aucune recommandation" subtext="Regardez plus de films"/> : <HScrollRow>{recommendations.map(f => <PortraitCard key={`rec-${f.id}`} item={f}/>)}</HScrollRow>}
  ...
  const renderTags = () => {
    ...
    if (taggedWorks.length===0) return (
      <View style={{ paddingTop:60 }}>
        <EmptyState icon="pricetag-outline" text="Aucune œuvre taguée" subtext={`Demandez à d'autres cinéastes de vous créditer\ndans leurs œuvres sur Universe.`}/>
        <View style={{ height:110 }}/>
      </View>
    );
    const userLabel  = profileData.display_name || profileData.username || (user as any)?.username || '';
    ...
    return (
      <View>
        <View style={[pg.reelStats, { marginBottom:0 }]}>
          <View style={pg.reelStat}><Text style={pg.reelStatV}>{taggedWorks.length}</Text><Text style={pg.reelStatL}>œuvres</Text></View>
          {asDirector.length > 0 && <><View style={pg.reelStatDiv}/><View style={pg.reelStat}><Text style={[pg.reelStatV,{color:C.gold}]}>{asDirector.length}</Text><Text style={pg.reelStatL}>réalisation</Text></View></>}
          {asCast.length > 0     && <><View style={pg.reelStatDiv}/><View style={pg.reelStat}><Text style={[pg.reelStatV,{color:C.primary}]}>{asCast.length}</Text><Text style={pg.reelStatL}>casting</Text></View></>}
        </View>
        {asDirector.length > 0 && (
          <>
