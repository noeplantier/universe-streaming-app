/**
 * app/(tabs)/profile.tsx — UNIVERSE · CREATOR EDITION v5
 *
 * ★ FIX NETLIFY : onAuthStateChange + retry auth + pas de useFocusEffect
 * ★ Avatar dynamique (photo ou monogramme selon profile.avatar_url)
 * ★ Gamification ultra-dynamique : streak, level-up toast, badges interactifs
 * ★ Web-safe : RN Image partout, composants inlinés, dimensions explicites
 */
import React, {
...
type GridTab = 0|1|2;
interface UserStats { watchCount:number; critiqueCount:number; favCount:number; watchedGenres:Record<string,number>; isNight:boolean; totalLikedLowPopularity:number; streak:number }
...
// ─── GAMIFICATION ─────────────────────────────────────────────────────────────
function buildMissions(s:UserStats):Mission[]{return[{id:'watch5',title:'Cinéphile actif',desc:'Regardez 5 œuvres',reward:'+15 pts',icon:'play-circle-outline',target:5,progress:Math.min(5,s.watchCount),completed:s.watchCount>=5},{id:'crit5',title:'Voix critique',desc:'5 critiques publiées',reward:'+40 pts + badge',icon:'create-outline',target:5,progress:Math.min(5,s.critiqueCount),completed:s.critiqueCount>=5},{id:'fav10',title:'Curateur passionné',desc:'10 favoris sauvegardés',reward:'+20 pts + badge',icon:'bookmark-outline',target:10,progress:Math.min(10,s.favCount),completed:s.favCount>=10},{id:'pepite3',title:'Chasseur de pépites',desc:'3 films rares aimés',reward:'+30 pts + badge',icon:'sparkles-outline',target:3,progress:Math.min(3,s.totalLikedLowPopularity),completed:s.totalLikedLowPopularity>=3}];}
function buildBadges(s:UserStats):Badge[]{return[{id:'explorer',label:'Explorateur indé',desc:'5 œuvres regardées',icon:'compass-outline',earned:s.watchCount>=5,pts:15},{id:'nocturne',label:'Cinéphile nocturne',desc:'Actif après 22h',icon:'moon-outline',earned:s.isNight,pts:5},{id:'pepiteur',label:'Découvreur de pépites',desc:'3 films rares aimés',icon:'sparkles-outline',earned:s.totalLikedLowPopularity>=3,pts:30},{id:'critique',label:'Critique en herbe',desc:'5 avis publiés',icon:'create-outline',earned:s.critiqueCount>=5,pts:40},{id:'curateur',label:'Curateur',desc:'10 favoris',icon:'bookmark-outline',earned:s.favCount>=10,pts:20},{id:'omnivore',label:'Cinéphile omnivore',desc:'5 genres explorés',icon:'layers-outline',earned:Object.keys(s.watchedGenres).length>=5,pts:25},{id:'streak',label:'Habitué',desc:'3 jours consécutifs',icon:'flame-outline',earned:s.streak>=3,pts:10}];}
...
function useGamification(userId:string){
  const[stats,setStats]=useState<UserStats>({watchCount:0,critiqueCount:0,favCount:0,watchedGenres:{},isNight:false,totalLikedLowPopularity:0,streak:0});
  ...
  useEffect(()=>{
    ...
    Promise.all([
      ...
      setStats(prev=>({...prev,watchCount:histIds.length,critiqueCount:(crit.data??[]).length,favCount:(favs.data??[]).length,isNight,streak:isNight?prev.streak+1:prev.streak}));
      setLoading(false);
  ...
  // Level-up detection
  useEffect(()=>{
    if(prevLevel>0&&level.n>prevLevel){setLevelUpVisible(true);setTimeout(()=>setLevelUpVisible(false),3500);}
    setPrevLevel(level.n);
  },[level.n]);
...
const LevelUpToast=memo(({visible,level}:{visible:boolean;level:number})=>{
  ...
  useEffect(()=>{if(visible){Animated.parallel([Animated.spring(ty,{toValue:0,tension:60,friction:10,useNativeDriver:true}),Animated.timing(op,{toValue:1,duration:300,useNativeDriver:true})]).start();setTimeout(()=>Animated.parallel([Animated.timing(ty,{toValue:-100,duration:300,useNativeDriver:true}),Animated.timing(op,{toValue:0,duration:300,useNativeDriver:true})]).start(),3000);}else{ty.setValue(-100);op.setValue(0);}},[visible]);
...
export default function ProfileScreen() {
  ...
  // ★ FIX NETLIFY — auth robuste avec onAuthStateChange
  useEffect(()=>{
    let mounted=true;
    const tryGetSession=async()=>{
      try{
        const{data:{session}}=await supabase.auth.getSession();
        if(mounted&&session?.user?.id){setUid(session.user.id);return;}
        // Web fallback: attend 600ms pour hydration localStorage
        await new Promise(r=>setTimeout(r,600));
        const{data:{session:s2}}=await supabase.auth.getSession();
        if(mounted&&s2?.user?.id)setUid(s2.user.id);
        else if(mounted)setLoading(false);
      }catch(e){console.error('[profile] auth:',e);if(mounted)setLoading(false);}
    };
    tryGetSession();
    const{data:{subscription}}=supabase.auth.onAuthStateChange((_,s)=>{
      if(!mounted)return;
      if(s?.user?.id)setUid(s.user.id);
      else{setUid(null);setLoading(false);}
    });
  ...
  const loadAll=useCallback(async(userId:string)=>{
    ...
    try{
      ...
      if(genres.length){const{data:recData}=await supabase.from('works').select('*').in('genre',genres).order('likes',{ascending:false}).limit(12);if(recData)setRecs(recData.map(mapWork).filter(w=>!seenIds.includes(w.id)));}
    }catch(e){console.error('[profile] fetch:',e);setFetchError(true);}
    finally{setLoading(false);setRefreshing(false);}
  },[]);

  // ★ FIX NETLIFY — useEffect au lieu de useFocusEffect
  useEffect(()=>{if(uid)loadAll(uid);},[uid]);

  // Realtime
  useEffect(()=>{
    if(!uid)return;
    const ts=Date.now();
    const ch1=supabase.channel(`rt_r_${ts}`).on('postgres_changes',{event:'UPDATE',schema:'public',table:'reels'},({new:r})=>{const u=mapReel(r);setReels(p=>p.map(x=>x.id===u.id?u:x));}).on('postgres_changes',{event:'INSERT',schema:'public',table:'reels'},({new:r})=>{const reel=mapReel(r);if(reel.id)setReels(p=>[reel,...p.filter(x=>x.id!==reel.id)]);}).subscribe();
    const ch2=supabase.channel(`rt_p_${ts}`).on('postgres_changes',{event:'UPDATE',schema:'public',table:'profiles'},({new:r})=>{if((r as any).id===uid)setProfile(mapProfile(r));}).subscribe();
    return()=>{supabase.removeChannel(ch1);supabase.removeChannel(ch2);};
  },[uid]);

  const{score,level,badges,missions,levelUpVisible}=useGamification(uid||'anonymous');
  ...
  const renderFilms=()=>{
    ...
    if(fetchError)return<View style={{alignItems:'center',paddingVertical:40,gap:12}}><Ionicons name="cloud-offline-outline" size={32} color={C.muted}/><Text style={{color:C.muted,fontSize:14,textAlign:'center',paddingHorizontal:40}}>Impossible de charger vos données.{'\n'}Vérifiez votre connexion.</Text><TouchableOpacity style={{paddingHorizontal:20,paddingVertical:10,borderRadius:13,backgroundColor:C.navyLow,borderWidth:StyleSheet.hairlineWidth,borderColor:C.border}} onPress={()=>uid&&loadAll(uid)}><Text style={{color:C.white,fontSize:13,fontWeight:'700'}}>Réessayer</Text></TouchableOpacity></View>;
    return(<View><SectionHeader icon="heart-outline" label="Œuvres favorites" count={favWorks.length} onViewAll={()=>router.push('/profile/favorites' as any)}/>{!favWorks.length?<EmptyState icon="heart-outline" text="Aucun favori" subtext="Sauvegardez des films depuis le catalogue"/>:<HScrollRow>{favWorks.map((f,i)=><PortraitCard key={f.id} item={f} rank={i+1}/>)}</HScrollRow>}<View style={{height:StyleSheet.hairlineWidth,backgroundColor:C.faint,marginTop:22}}/><SectionHeader icon="create-outline" label="Mes critiques" subtitle="Par popularité" onViewAll={()=>router.push('/profile/reviews' as any)}/>{!reviews.length?<EmptyState icon="chatbubble-outline" text="Aucune critique"/>:<HScrollRow>{reviews.map((r,i)=><CritiqueCard key={r.id} review={r} rank={i+1} onPress={()=>router.push(`/review/${r.id}` as any)}/>)}</HScrollRow>}<View style={{height:StyleSheet.hairlineWidth,backgroundColor:C.faint,marginTop:22}}/><SectionHeader icon="eye-outline" label="Œuvres visionnées" onViewAll={()=>router.push('/profile/seen_films' as any)}/>{!watched.length?<EmptyState icon="film-outline" text="Aucun visionnage"/>:<HScrollRow>{watched.map((f,i)=><PortraitCard key={f.id} item={f} rank={i+1}/>)}</HScrollRow>}<View style={{height:StyleSheet.hairlineWidth,backgroundColor:C.faint,marginTop:22}}/><SectionHeader icon="shuffle-outline" label="Recommandés" subtitle="Basé sur vos goûts"/>{!recs.length?<EmptyState icon="planet-outline" text="Aucune recommandation"/>:<HScrollRow>{recs.map(f=><PortraitCard key={f.id} item={f}/>)}</HScrollRow>}<View style={{height:110}}/></View>);
  };
  ...
  return(
    <View style={{flex:1,backgroundColor:C.bg}}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={()=>{setRefreshing(true);if(uid)loadAll(uid);}} tintColor={C.mid}/>}>
