 ...
 *
 * ✦ ZERO supabase.auth.* — UUID device via getDeviceId() (gratuit, sans 402/500)
 * ✦ useFocusEffect → re-fetch automatique à chaque retour sur l'onglet
 * ✦ Tables : user_favorites · critiques (+ fallback v_user_critiques)
 *            user_history (+ fallback seen_films/user_seen_films)
 * ✦ Algo reco : genre weights favoris+3, vus+1, critiques≥3 positives
 * ✦ UX/UI identique — zéro régression visuelle
 */
import React, {
...
const mapReview = (r:any): ReviewItem => ({ id:String(r?.id??''), content:String(r?.content??r?.contenu??''), rating:r?.rating!=null?Number(r.rating):r?.note!=null?Number(r.note):0, likes:Number(r?.likes_count??0), film:{ id:String(r?.reel_id??r?.work_id??r?.id), title:String(r?.film_title??r?.work_title??r?.title??r?.titre??'—'), genre:r?.work_genre??r?.genre??'—' } });
...
// ─── FETCH HELPERS — resilients aux noms de tables multiples ──────────────────

/**
 * Critiques : essaie v_user_critiques (vue enrichie) puis critiques (table).
 * Les deux retournent des colonnes compatibles avec mapReview.
 */
async function fetchCritiques(uid: string): Promise<ReviewItem[]> {
  // Tentative 1 : vue v_user_critiques (enrichie avec genre du film)
  // Si la vue n'existe pas encore (avant exécution de universe_setup.sql) → fallback
  try {
    const { data: v, error: vErr } = await supabase
      .from('v_user_critiques')
  ...
  } catch { /* vue absente → fallback */ }

  // Tentative 2 : table critiques directe (toujours disponible)
  try {
...
/**
 * Œuvres visionnées : essaie seen_films/user_seen_films (film_id)
 * puis user_history (work_id).
 */
async function fetchSeen(uid: string): Promise<{ workId:number; watchedAt:string }[]> {
  // Tentative 1 : vue seen_films (film_id)
  ...
  if (!sfErr && sf) {
    const items = sf.map((r:any) => ({ workId: Number(r.film_id), watchedAt: r.created_at ?? '' })).filter(r => r.workId > 0);
    if (items.length > 0) return items;
  }

  // Tentative 2 : table user_seen_films (film_id)
  const { data: usf, error: usfErr } = await supabase
    .from('user_seen_films')
    .select('film_id,created_at')
    .eq('user_id', uid);

  if (!usfErr && usf) {
    const items = usf.map((r:any) => ({ workId: Number(r.film_id), watchedAt: r.created_at ?? '' })).filter(r => r.workId > 0);
    if (items.length > 0) return items;
  }

  // Tentative 3 : table user_history (work_id)
...
// ─── GAMIFICATION ─────────────────────────────────────────────────────────────
function buildBadges(s:GamiStats): Badge[] { return [ {id:'explorer',label:'Explorateur',icon:'compass-outline',earned:s.watchCount>=5,pts:15}, {id:'nocturne',label:'Insomniaque',icon:'moon-outline',earned:s.isNight,pts:5}, {id:'critique',label:'Critique',icon:'create-outline',earned:s.critiqueCount>=5,pts:40}, {id:'curateur',label:'Curateur',icon:'bookmark-outline',earned:s.favCount>=10,pts:20}, {id:'omnivore',label:'Omnivore',icon:'layers-outline',earned:s.watchCount>=15,pts:25}, {id:'streak',label:'Habitué',icon:'flame-outline',earned:s.streak>=3,pts:10}, ]; }
function buildMissions(s:GamiStats): Mission[] { return [ {id:'watch5',title:'Cinéphile actif',desc:'5 œuvres visionnées',reward:'+15 pts',icon:'play-circle-outline',target:5,progress:Math.min(5,s.watchCount),completed:s.watchCount>=5}, {id:'crit5',title:'Voix critique',desc:'5 critiques publiées',reward:'+40 pts + badge',icon:'create-outline',target:5,progress:Math.min(5,s.critiqueCount),completed:s.critiqueCount>=5}, {id:'fav10',title:'Curateur passionné',desc:'10 favoris',reward:'+20 pts + badge',icon:'bookmark-outline',target:10,progress:Math.min(10,s.favCount),completed:s.favCount>=10}, ]; }
function cinephileLevel(score:number) { const L=[{at:0,n:1,l:'Spectateur curieux'},{at:50,n:2,l:'Explorateur indé'},{at:150,n:3,l:'Critique amateur'},{at:400,n:4,l:'Curateur underground'},{at:900,n:5,l:'Ambassadeur cinéma'}]; const cur=[...L].reverse().find(x=>score>=x.at)??L[0]; const ni=L.findIndex(x=>x.n===cur.n)+1; const nxt=L[ni]??L[L.length-1]; return{n:cur.n,label:cur.l,pct:cur.n===5?1:Math.min(1,(score-cur.at)/(nxt.at-cur.at)),nextAt:nxt.at}; }
...
function useGamification(uid:string|null) {
  ...
  useEffect(()=>{
    ...
    Promise.all([
      ...
      watchCount:h.count??0, critiqueCount:c.count??0, favCount:f.count??0,
...
// ─── MICRO UI ─────────────────────────────────────────────────────────────────
const Shimmer=memo(({w,h,r=8}:{w:number;h:number;r?:number})=>{const op=useRef(new Animated.Value(0.12)).current;useEffect(()=>{const l=Animated.loop(Animated.sequence([Animated.timing(op,{toValue:0.30,duration:900,useNativeDriver:true}),Animated.timing(op,{toValue:0.12,duration:900,useNativeDriver:true})]));l.start();return()=>l.stop();},[op]);return<Animated.View style={{width:w,height:h,borderRadius:r,backgroundColor:C.navyMid,opacity:op}}/>;});
...
const SecHead=memo(({icon,label,count,onMore}:{icon:keyof typeof Ionicons.glyphMap;label:string;count?:number;onMore?:()=>void})=>(<View style={{flexDirection:'row',alignItems:'center',paddingHorizontal:H_PAD,paddingTop:22,paddingBottom:12,gap:7}}><Ionicons name={icon} size={13} color={C.mid}/><Text style={{color:C.white,fontSize:15,fontWeight:'800',letterSpacing:-0.2,flex:1}}>{label}</Text>{count!=null&&<View style={sh.pill}><Text style={sh.pillTxt}>{count}</Text></View>}{onMore&&<TouchableOpacity onPress={onMore} hitSlop={8} style={sh.btn}><Text style={sh.btnTxt}>Tout voir</Text><Ionicons name="chevron-forward" size={11} color={C.blue}/></TouchableOpacity>}</View>));
const sh=StyleSheet.create({pill:{paddingHorizontal:7,paddingVertical:2,borderRadius:7,backgroundColor:C.navyMid,borderWidth:StyleSheet.hairlineWidth,borderColor:C.border},pillTxt:{color:C.muted,fontSize:9,fontWeight:'700'},btn:{flexDirection:'row',alignItems:'center',gap:3,paddingHorizontal:10,paddingVertical:5,borderRadius:10,backgroundColor:C.blueDim,borderWidth:StyleSheet.hairlineWidth,borderColor:'rgba(90,150,230,0.20)'},btnTxt:{color:C.blue,fontSize:11,fontWeight:'700'}});
...
// ─── PROFILE HEADER ───────────────────────────────────────────────────────────
interface HeaderProps { profile:ProfileData; filmCount:number; critiqueCount:number; reelCount:number; level:ReturnType<typeof cinephileLevel>; score:number; unreadNotifs:number; streak:number; onEdit:()=>void; onAdmin:()=>void; onNotifs:()=>void; onSettings:()=>void }

const ProfileHeader=memo(function ProfileHeader({profile,filmCount,critiqueCount,reelCount,level,score,unreadNotifs,streak,onEdit,onAdmin,onNotifs,onSettings}:HeaderProps){
  ...
  useEffect(()=>{
    ...
    Animated.loop(Animated.sequence([Animated.timing(glowAnim,{toValue:1,duration:2200,useNativeDriver:true}),Animated.timing(glowAnim,{toValue:0.4,duration:2200,useNativeDriver:true})])).start();
    Animated.loop(Animated.sequence([Animated.timing(ringAnim,{toValue:1,duration:3000,useNativeDriver:true}),Animated.timing(ringAnim,{toValue:0,duration:3000,useNativeDriver:true})])).start();
  },[level.pct]);
  const dn=profile.display_name||profile.username||'Cinéphile';
  const init=dn.trim().split(/\s+/).map(n=>n[0]).join('').toUpperCase().slice(0,2);
  const links=[{k:'ig',icon:'logo-instagram' as const,url:profile.social_instagram,label:'Instagram'},{k:'vi',icon:'videocam-outline' as const,url:profile.social_vimeo,label:'Vimeo'},{k:'yt',icon:'logo-youtube' as const,url:profile.social_youtube,label:'YouTube'},{k:'ws',icon:'globe-outline' as const,url:profile.website,label:'Portfolio'}].filter(l=>!!l.url);
  ...
  return(
    <View>
      ...
      <View style={ph.topNav}>
        <View style={ph.navLeft}><Text style={ph.brandTxt}>UNIVERSE</Text><View style={ph.navDot}/><Text style={ph.navSub}>{ROLE_LABELS[profile.role]??'Cinéaste'}</Text></View>
        <View style={ph.navRight}>
          <TouchableOpacity style={ph.navBtn} onPress={onEdit} activeOpacity={0.75}><Ionicons name="create-outline" size={16} color={C.offWhite}/></TouchableOpacity>
          <TouchableOpacity style={ph.navBtn} onPress={onAdmin} activeOpacity={0.75}><Ionicons name="eye-outline" size={17} color={C.offWhite}/></TouchableOpacity>
          <TouchableOpacity style={ph.navBtn} onPress={onNotifs} activeOpacity={0.75}>
            <Ionicons name="notifications-outline" size={18} color={C.offWhite}/>
            {unreadNotifs>0&&<View style={ph.notifBadge}><Text style={ph.notifBadgeTxt}>{unreadNotifs>9?'9+':unreadNotifs}</Text></View>}
          </TouchableOpacity>
          <TouchableOpacity style={ph.navBtn} onPress={onSettings} activeOpacity={0.75}><Ionicons name="settings-outline" size={17} color={C.offWhite}/></TouchableOpacity>
        </View>
      </View>
      {/* Avatar + stats */}
      <View style={ph.identityRow}>
        <View style={{position:'relative',marginRight:18}}>
          <Animated.View style={[ph.ringOuter,{transform:[{scale:ringScale}],opacity:glowAnim}]}/>
          <View style={ph.avatarWrap}>
            {profile.avatar_url&&!imgErr?<Image source={{uri:profile.avatar_url}} style={ph.avatar} resizeMode="cover" onError={()=>setImgErr(true)}/>:<View style={[ph.avatar,ph.avatarFb]}><Text style={ph.avatarInit}>{init}</Text></View>}
            <View style={ph.levelBadge}><Text style={ph.levelBadgeTxt}>{level.n}</Text></View>
            {profile.is_pro&&<View style={ph.proBadge}><Text style={ph.proBadgeTxt}>PRO</Text></View>}
          </View>
        </View>
        <View style={{flex:1,gap:6}}>
          <View style={{flexDirection:'row',gap:6}}>
            {[{val:fmt(filmCount),label:'films'},{val:fmt(critiqueCount),label:'critiques'},{val:fmt(reelCount),label:'créas'}].map(chip=>(<View key={chip.label} style={ph.chip}><Text style={ph.chipVal}>{chip.val}</Text><Text style={ph.chipLbl}>{chip.label}</Text></View>))}
          </View>
          <View style={{flexDirection:'row',justifyContent:'space-between',alignItems:'center'}}>
            <Text style={ph.levelLbl}>Niv.{level.n} · {level.label}</Text>
            <Text style={{color:C.muted,fontSize:9,fontWeight:'700'}}>{fmt(score)} pts</Text>
          </View>
          <View style={ph.levelTrack}><Animated.View style={[ph.levelFill,{width:levelBarW}]}/></View>
          {level.n<5&&<Text style={{color:C.muted,fontSize:8.5,fontWeight:'600'}}>{fmt(Math.max(0,level.nextAt-score))} pts → Niv.{level.n+1}</Text>}
        </View>
      </View>
      {/* Nom */}
      <View style={ph.nameRow}>
        <Text style={ph.displayName} numberOfLines={1}>{dn}</Text>
        {profile.is_industry_contact&&<View style={ph.indBadge}><Ionicons name="briefcase-outline" size={9} color={C.mid}/><Text style={ph.indTxt}>INDUSTRIE</Text></View>}
        {streak>=3&&<View style={ph.streakBadge}><Ionicons name="flame-outline" size={9} color={C.gold}/><Text style={[ph.indTxt,{color:C.gold}]}>{streak}J</Text></View>}
      </View>
      {(profile.location||profile.username)&&<View style={{flexDirection:'row',alignItems:'center',gap:5,paddingHorizontal:H_PAD,marginBottom:10}}>{profile.location&&<Ionicons name="location-outline" size={11} color={C.muted}/>}<Text style={ph.subline}>{[profile.username&&`@${profile.username}`,profile.location].filter(Boolean).join(' · ')}</Text></View>}
      {!!profile.bio&&<Pressable onPress={()=>setBioExpanded(e=>!e)} style={{paddingHorizontal:H_PAD,marginBottom:12}}><Text style={ph.bio} numberOfLines={bioExpanded?undefined:2}>{profile.bio}</Text>{profile.bio.length>90&&<Text style={ph.bioToggle}>{bioExpanded?'Moins ↑':'Suite ↓'}</Text>}</Pressable>}
      {links.length>0&&<ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{paddingHorizontal:H_PAD,gap:7,marginTop:10,paddingBottom:2}}>{links.map(l=>(<TouchableOpacity key={l.k} style={ph.socialBtn} onPress={()=>Linking.openURL(l.url!).catch(()=>{})} activeOpacity={0.78}><BlurView intensity={Platform.OS==='ios'?12:8} tint="dark" style={StyleSheet.absoluteFill}/><Ionicons name={l.icon} size={13} color={C.offWhite}/><Text style={ph.socialTxt}>{l.label}</Text></TouchableOpacity>))}</ScrollView>}
      <View style={{height:StyleSheet.hairlineWidth,backgroundColor:C.faint,marginTop:16}}/>
    </View>
  );
});
...
const ph=StyleSheet.create({
  ...
  navBtn:{width:34,height:34,borderRadius:17,alignItems:'center',justifyContent:'center',backgroundColor:C.navyLow,borderWidth:StyleSheet.hairlineWidth,borderColor:C.border},
  ...
  ringOuter:{position:'absolute',top:-5,left:-5,width:90,height:90,borderRadius:45,borderWidth:1.5,borderColor:C.ring},
  avatarWrap:{position:'relative'},
  avatar:{width:80,height:80,borderRadius:40,backgroundColor:C.navyMid},
  avatarFb:{alignItems:'center',justifyContent:'center'},
  avatarInit:{color:C.white,fontSize:24,fontWeight:'900',letterSpacing:-0.5},
  ...
  chip:{flex:1,alignItems:'center',gap:1,paddingVertical:7,borderRadius:11,backgroundColor:C.navyLow,borderWidth:StyleSheet.hairlineWidth,borderColor:C.border},
  chipVal:{color:C.white,fontSize:16,fontWeight:'900',letterSpacing:-0.5},
  chipLbl:{color:C.muted,fontSize:7.5,fontWeight:'700',textTransform:'uppercase',letterSpacing:0.5},
  levelLbl:{color:C.muted,fontSize:9.5,fontWeight:'600'},
  ...
  indBadge:{flexDirection:'row',alignItems:'center',gap:3,paddingHorizontal:7,paddingVertical:2.5,borderRadius:6,borderWidth:StyleSheet.hairlineWidth,borderColor:C.border,backgroundColor:C.navyMid},
  indTxt:{color:C.muted,fontSize:7.5,fontWeight:'800',letterSpacing:0.5},
...
const IBadge=memo(({b}:{b:Badge})=>{const[open,setOpen]=useState(false),sc=useRef(new Animated.Value(1)).current;const press=()=>{Animated.sequence([Animated.spring(sc,{toValue:0.88,tension:300,friction:7,useNativeDriver:true}),Animated.spring(sc,{toValue:1,tension:200,friction:8,useNativeDriver:true})]).start();setOpen(v=>!v);};return(<Animated.View style={{transform:[{scale:sc}]}}><TouchableOpacity onPress={press} activeOpacity={0.85} style={[ib.w,b.earned&&ib.e]}><View style={[ib.ic,b.earned&&ib.eo]}><Ionicons name={b.icon} size={16} color={b.earned?C.white:C.muted}/></View><Text style={[ib.l,b.earned&&{color:C.white}]} numberOfLines={2}>{b.label}</Text>{b.earned&&<Text style={{color:C.gold,fontSize:8,fontWeight:'800'}}>+{b.pts}pts</Text>}{!b.earned&&<View style={{position:'absolute',top:7,right:7}}><Ionicons name="lock-closed" size={7} color={C.muted}/></View>}</TouchableOpacity></Animated.View>);});
...
export default function ProfileScreen() {
  ...
  // ── ★ useFocusEffect — re-fetch à chaque retour sur l'onglet ─────────────
  // Garantit que favoris / critiques / vus apparaissent immédiatement
  // après une action effectuée dans un autre screen.
  useFocusEffect(
    useCallback(() => {
  ...
  const loadStreak = useCallback(async (userId: string) => {
    try {
      // user_history en priorité
      const { data } = await supabase.from('user_history')
        .select('watched_at').eq('user_id', userId)
        .order('watched_at', { ascending:false }).limit(30);
      const rows = data ?? [];
      if (!rows.length) return;
      let s = 1;
      for (let i = 1; i < rows.length; i++) {
        const gap = new Date(rows[i-1].watched_at).getTime() - new Date(rows[i].watched_at).getTime();
  ...
  // ── ★ loadAll — 3 phases parallèles ─────────────────────────────────────
  const loadAll = useCallback(async (userId: string) => {
    if (!userId) return;
    setLoading(true); setFErr(false);

    try {
      // ── Phase 1 : toutes les données user en parallèle ───────────────────
      const [profR, reelsR, critiques, favR, seenItems] = await Promise.all([
        ...
        // Critiques : v_user_critiques puis fallback critiques
        fetchCritiques(userId),

        // Favoris
        supabase.from('user_favorites').select('work_id').eq('user_id', userId),

        // Films vus : seen_films → user_seen_films → user_history
        fetchSeen(userId),
      ]);
      ...
      // ── Phase 2 : fetch works favoris + vus en parallèle ─────────────────
      ...
      //  Favoris       → genre +3, catégorie +1.5  (signal fort)
      //  Vus           → genre +1                  (signal neutre)
      //  Critiques ≥ 3 → genre +rating/5*2         (signal qualitatif)
      ...
      critiques.forEach(c => {
        if (c.rating < 3) return;
        if (c.film?.genre && c.film.genre !== '—')
          genreW[c.film.genre] = (genreW[c.film.genre] ?? 0) + (c.rating / 5) * 2;
      });
    ...
    return (
      <View style={{marginTop:16}}>
        <Accordion icon="person-circle-outline" title="Identité cinématographique" defaultOpen badge={ROLE_LABELS[profile.role]??'Cinéaste'}>
          <View style={{flexDirection:'row',flexWrap:'wrap',gap:7}}>{[ROLE_LABELS[profile.role]??'Cinéaste',...profile.specialties].map((s,i)=>(<View key={i} style={{paddingHorizontal:11,paddingVertical:6,borderRadius:18,borderWidth:StyleSheet.hairlineWidth,borderColor:i===0?C.borderHi:C.border,backgroundColor:i===0?C.subtle:C.faint}}><Text style={{color:i===0?C.white:C.mid,fontSize:11,fontWeight:i===0?'700':'500'}}>{s}</Text></View>))}</View>
        ...
        <Accordion icon="link-outline" title="Ouvert à collaborer">
          {profile.open_to.length===0?<Text style={{color:C.muted,fontSize:12,textAlign:'center',paddingVertical:8}}>Précisez vos disponibilités dans "Modifier le profil"</Text>:<View style={{flexDirection:'row',flexWrap:'wrap',gap:7}}>{profile.open_to.map(col=>(<View key={col} style={{flexDirection:'row',alignItems:'center',gap:5,paddingHorizontal:11,paddingVertical:6,borderRadius:18,borderWidth:StyleSheet.hairlineWidth,borderColor:C.blueDim,backgroundColor:'rgba(90,150,230,0.06)'}}><Ionicons name="checkmark-circle-outline" size={11} color={C.blue}/><Text style={{color:C.blue,fontSize:11,fontWeight:'600'}}>{col}</Text></View>))}</View>}
        </Accordion>
        <View style={{height:110}}/>
      </View>
    );
  ...
  const renderCreations = () => {
    if (loading) return <View><SkeletonSection/></View>;
    if (!reels.length) return (<View style={{paddingTop:50,paddingHorizontal:H_PAD}}><Empty icon="videocam-outline" text="Aucune création" sub="Importez vos vidéos depuis l'onglet Créer"/><TouchableOpacity style={{flexDirection:'row',alignItems:'center',justifyContent:'center',gap:7,marginTop:16,borderRadius:11,borderWidth:StyleSheet.hairlineWidth,borderColor:C.border,backgroundColor:C.navyLow,paddingVertical:13}} onPress={()=>router.push('/(tabs)/create' as any)} activeOpacity={0.85}><Ionicons name="add-circle-outline" size={14} color={C.mid}/><Text style={{color:C.mid,fontSize:12.5,fontWeight:'700'}}>Importer une vidéo</Text></TouchableOpacity><View style={{height:110}}/></View>);
  ...
  return (
    <View style={{flex:1,backgroundColor:C.bg}}>
      ...
      <GalaxyBackground/>
      <ScrollView showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={()=>{setRef(true);if(uid){loadAll(uid);loadUnread(uid);}}} tintColor={C.mid}/>}>
        <SafeAreaView edges={['top']}>
          <ProfileHeader profile={profile} filmCount={watched.length} critiqueCount={reviews.length} reelCount={reels.length} level={level} score={score} unreadNotifs={unreadNotifs} streak={streak} onEdit={nav.edit} onAdmin={nav.admin} onNotifs={nav.notifs} onSettings={nav.settings}/>
        </SafeAreaView>
        ...
        <View style={{marginTop:16,gap:12,marginBottom:4}}>
          ...
          <View style={{paddingHorizontal:H_PAD,gap:8}}>
            <View style={{flexDirection:'row',alignItems:'center',gap:7,marginBottom:2}}>
              <Ionicons name="flag-outline" size={12} color={C.mid}/>
