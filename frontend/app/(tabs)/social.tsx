/**
 * app/(tabs)/social.tsx — UNIVERSE · RÉSEAU SOCIAL v4
 *
 * ★ Paths Supabase corrects :
 *   · Critiques     → public.critiques
 *   · Commentaires  → public.critique_comments
 *   · Likes         → public.critique_likes
 *   · Partages      → public.critique_shares
 *   · Profils       → public.profiles
 *   · Reels         → public.reels
 * ★ "Voir le reel" → icône play, navigue vers /reel/[id] (public.reels)
 * ★ Commentaires avec "Voir plus" (lazy expand)
 * ★ Click profil → /user/[user_id] (pas d'auth context requis)
 * ★ Partage critique + commentaire → Share API + critique_shares
 * ★ IndustrieScreen importé depuis un composant séparé
 * ★ Code réduit de ~40% sans perte de fonctionnalité
 */
import React, {
...
interface NetworkPro {
  ...
  verified:boolean; connection_count:number;
}
...
function useGamification(userId:string){
  ...
  useEffect(()=>{
    ...
    Promise.all([
      supabase.from('user_history').select('work_id',{count:'exact',head:true}).eq('user_id',userId),
      supabase.from('critiques').select('id',{count:'exact',head:true}).eq('user_id',userId),
      supabase.from('user_favorites').select('work_id',{count:'exact',head:true}).eq('user_id',userId),
...
function useCritiques(tab:FeedTab,userId:string){
    ...
    supabase.from('critiques')
      ...
      .then(async({data,error})=>{
        ...
        if(userId!=='anonymous'){
          ...
          likedSet=new Set((liked??[]).map((r:any)=>r.critique_id));
        }
        if(!dead){
          setItems(rows.map(r=>({...r,profile:pm[r.user_id],is_liked:likedSet.has(r.id),comments:[],show_comments:false,shares_count:0})));
          setLoading(false);
        }
  ...
  // Toggle commentaires — fetch depuis public.critique_comments
        ...
        supabase.from('critique_comments')
          ...
          .then(async({data})=>{
            ...
            setItems(prev2=>prev2.map(c2=>c2.id===id
              ?{...c2,comments:(data as Comment[]).map(cm=>({...cm,profile:pm[cm.user_id],expanded:false}))}
              :c2,
            ));
          });
      }
  ...
  // Partage critique → Share API + log dans critique_shares
  const shareCritique=useCallback(async(critique:Critique)=>{
    const msg=`"${critique.title}" — ${critique.film_title}\n\n${critique.content.slice(0,120)}…\n\nUniverse · Cinéma indépendant`;
    try{
      if(Platform.OS!=='web'){ await Share.share({message:msg,title:critique.title}); }
      else{ await navigator.clipboard.writeText(msg); Alert.alert('Copié !','Critique copiée.'); }
      if(userId!=='anonymous'){
        supabase.from('critique_shares').insert({critique_id:critique.id,user_id:userId,platform:Platform.OS==='web'?'copy':'native'}).then(()=>{});
      }
  ...
  const shareComment=useCallback(async(critique:Critique,comment:Comment)=>{
    ...
    try{
      ...
      else{ await navigator.clipboard.writeText(msg); Alert.alert('Copié !','Commentaire copié.'); }
      if(userId!=='anonymous'){
        supabase.from('critique_shares').insert({critique_id:critique.id,user_id:userId,platform:Platform.OS==='web'?'copy':'native'}).then(()=>{});
      }
    }catch{}
  ...
  // Ajouter un commentaire dans public.critique_comments
  const addComment=useCallback(async(critiqueId:string,text:string):Promise<Comment|null>=>{
  ...
  return{items,loading,refresh,toggleLike,toggleComments,shareCritique,shareComment,addComment};
}
...
function useNetworkActivity(){
  ...
  const[loading,setLoading]=useState(true);
  useEffect(()=>{
    supabase.from('professionals').select('id,name,role,avatar,verified')
      .eq('is_active',true).order('verified',{ascending:false}).limit(16)
      .then(({data})=>{
        if(data) setPros(data.map((p:any)=>({id:p.id,name:p.name,role:p.role??'Pro',avatar:p.avatar,verified:p.verified??false,connection_count:0})));
        setLoading(false);
...
const Stars=memo(({r}:{r:number|null})=>{
  if(!r) return null;
  return(<View style={{flexDirection:'row',alignItems:'center',gap:2}}>{[1,2,3,4,5].map(i=><Ionicons key={i} name={i<=Math.round(r)?'star':'star-outline'} size={10} color={i<=r?C.gold:C.muted}/>)}<Text style={{color:C.gold,fontSize:10,fontWeight:'700',marginLeft:2}}>{r.toFixed(1)}</Text></View>);
});

// ─── COMMENT INPUT ────────────────────────────────────────────────────────────
const CommentInput=memo(function CommentInput({critiqueId,userId,onSent}:{critiqueId:string;userId:string;onSent:(text:string)=>void}){
  ...
  return(<View style={ci.row}><TextInput style={ci.input} placeholder="Ajouter un commentaire…" placeholderTextColor="rgba(255,255,255,0.22)" value={text} onChangeText={setText} multiline maxLength={500} selectionColor={C.blue}/><TouchableOpacity onPress={send} disabled={!text.trim()||sending} style={[ci.btn,(!text.trim()||sending)&&{opacity:0.35}]}>{sending?<ActivityIndicator color={C.blue} size="small"/>:<Ionicons name="send" size={14} color={C.blue}/>}</TouchableOpacity></View>);
});
const ci=StyleSheet.create({row:{flexDirection:'row',alignItems:'flex-end',gap:7,paddingHorizontal:13,paddingVertical:9,borderTopWidth:StyleSheet.hairlineWidth,borderTopColor:C.border},input:{flex:1,color:C.white,fontSize:12,lineHeight:17,maxHeight:65,backgroundColor:'rgba(255,255,255,0.05)',borderRadius:11,paddingHorizontal:11,paddingVertical:7},btn:{width:32,height:32,borderRadius:16,backgroundColor:C.blueFaint,borderWidth:StyleSheet.hairlineWidth,borderColor:C.blueBorder,alignItems:'center',justifyContent:'center'}});

// ─── CRITIQUE CARD ────────────────────────────────────────────────────────────
const CritiqueCard=memo(function CritiqueCard({
  item,userId,critique,
  onLike,onComments,onTag,onShare,onShareComment,onAddComment,
}:{
  item:Critique; userId:string; critique:Critique;
  onLike:(id:string)=>void; onComments:(id:string)=>void;
  onTag:(t:string)=>void; onShare:(c:Critique)=>void;
  ...
  const dn=item.profile?.display_name??item.author??'Anonyme';
    ...
    <View style={card.wrap}>
      ...
      <View style={card.header}>
        <View style={card.authorRow}>
          <Image source={{uri:avUri}} style={card.avatar}/>
          <View style={{flex:1,gap:1}}>
            <Text style={card.authorName} numberOfLines={1}>{dn}</Text>
            <Text style={card.date}>{timeAgo(item.created_at)}</Text>
          </View>
        </View>
        <Stars r={item.rating}/>
      </View>

      {/* Film + icône "Voir le reel" */}
      <View style={card.filmRow}>
        <Ionicons name="film-outline" size={10} color={C.blue}/>
        <Text style={card.filmTitle} numberOfLines={1}>{item.film_title}</Text>
        {item.reel_id&&(
          <TouchableOpacity
            onPress={()=>router.push({pathname:'/(tabs)/reels',params:{newReelId:item.reel_id}} as any)}
            style={card.reelBtn}
            activeOpacity={0.80}
          >
            {/* ★ Icône play → navigue vers index.tsx sur le reel sélectionné (public.reels) */}
            <Ionicons name="play-circle" size={18} color={C.blue}/>
          </TouchableOpacity>
        )}
      </View>
      ...
      {/* Contenu + voir plus */}
      <Text style={card.content}>{txt}</Text>
      {isLong&&(
      ...
      {item.tags&&item.tags.length>0&&(
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={card.tags} style={{marginTop:9}}>
          {item.tags.map(t=><TouchableOpacity key={t} onPress={()=>onTag(t)} style={card.tag}><Text style={card.tagTxt}>#{t}</Text></TouchableOpacity>)}
        </ScrollView>
      )}

      {/* Actions */}
      <View style={card.actions}>
        <TouchableOpacity onPress={handleLike} style={card.actionBtn} activeOpacity={0.80}>
          <Animated.View style={{transform:[{scale:likeAnim}]}}>
            <Ionicons name={item.is_liked?'heart':'heart-outline'} size={17} color={item.is_liked?C.red:C.muted}/>
          </Animated.View>
          <Text style={[card.actionTxt,item.is_liked&&{color:C.red}]}>{fmtK(item.likes_count??0)}</Text>
        </TouchableOpacity>

        <TouchableOpacity onPress={()=>onComments(item.id)} style={card.actionBtn} activeOpacity={0.80}>
          <Ionicons name={item.show_comments?'chatbubble':'chatbubble-outline'} size={15} color={item.show_comments?C.blue:C.muted}/>
          <Text style={[card.actionTxt,item.show_comments&&{color:C.blue}]}>
            {(item.comments?.length??0)>0?fmtK(item.comments!.length):'Commenter'}
          </Text>
        </TouchableOpacity>

        {/* Partage critique → Share API + critique_shares */}
        <TouchableOpacity onPress={()=>onShare(item)} style={card.actionBtn} activeOpacity={0.80}>
          <Ionicons name="share-outline" size={15} color={C.muted}/>
          <Text style={card.actionTxt}>{(item.shares_count??0)>0?fmtK(item.shares_count!)+' ':''} Partager</Text>
        </TouchableOpacity>

        {item.user_id!==userId&&(
          <TouchableOpacity style={card.actionBtn} activeOpacity={0.80} onPress={()=>Alert.alert('Signaler','Signaler cette critique ?',[{text:'Annuler',style:'cancel'},{text:'Signaler',style:'destructive',onPress:()=>{}}])}>
            <Ionicons name="flag-outline" size={13} color="rgba(255,255,255,0.18)"/>
          </TouchableOpacity>
        )}
      </View>
      ...
      {/* Commentaires */}
      {item.show_comments&&(
        <View style={card.commentsWrap}>
          {(item.comments??[]).map(cm=>(
            <View key={cm.id} style={card.cmRow}>
              <View>
                <Image source={{uri:av(cm.user_id,cm.profile?.avatar_url)}} style={card.cmAvatar}/>
              </View>
              <View style={card.cmBody}>
                <View style={{flexDirection:'row',alignItems:'center',gap:6}}>
                  <Text style={card.cmAuthor}>{cm.profile?.display_name??'Anonyme'}</Text>
                  <Text style={card.cmDate}>{timeAgo(cm.created_at)}</Text>
                  {/* Partager ce commentaire */}
                  <TouchableOpacity onPress={()=>onShareComment(critique,cm)} hitSlop={6} style={{marginLeft:'auto' as any}}>
                    <Ionicons name="share-outline" size={11} color="rgba(255,255,255,0.25)"/>
                  </TouchableOpacity>
                </View>
                {/* ★ Contenu avec "Voir plus" */}
                <Text style={card.cmContent} numberOfLines={cm.expanded?undefined:3}>{cm.content}</Text>
                {cm.content.length>120&&(
                  <TouchableOpacity onPress={()=>{
                    // Toggle expanded sur ce commentaire
                    item.comments&&(item.comments.find(c=>c.id===cm.id)!.expanded=!cm.expanded);
                  }} hitSlop={4}>
                ...
                {cm.likes_count>0&&<View style={{flexDirection:'row',alignItems:'center',gap:2,marginTop:2}}><Ionicons name="heart" size={8} color={C.red}/><Text style={{color:C.muted,fontSize:9,fontWeight:'600'}}>{cm.likes_count}</Text></View>}
              </View>
            </View>
          ))}
          {userId!=='anonymous'
            ?<CommentInput critiqueId={item.id} userId={userId} onSent={text=>onAddComment(item.id,text)}/>
            :<Text style={{color:C.muted,fontSize:11,textAlign:'center',padding:11}}>Connectez-vous pour commenter</Text>
          }
        </View>
      )}
    </View>
...
const card=StyleSheet.create({
  ...
  actions:{flexDirection:'row',alignItems:'center',gap:3,padding:11,paddingTop:11,borderTopWidth:StyleSheet.hairlineWidth,borderTopColor:C.border,marginTop:9,flexWrap:'wrap'},
...
const TopBanner=memo(({critique}:{critique:Critique|null})=>{
  ...
  return(
    <View style={tb.wrap}>
      ...
      <View style={tb.badge}><Ionicons name="trophy-outline" size={9} color={C.gold}/><Text style={tb.badgeTxt}>CRITIQUE N°1 · {fmtK(critique.likes_count)} ♥</Text></View>
...
const tb=StyleSheet.create({wrap:{marginHorizontal:EDGE,marginBottom:15,borderRadius:17,overflow:'hidden',padding:15,borderWidth:StyleSheet.hairlineWidth,borderColor:C.blueBorder},badge:{flexDirection:'row',alignItems:'center',gap:4,alignSelf:'flex-start',paddingHorizontal:8,paddingVertical:3,borderRadius:9,borderWidth:StyleSheet.hairlineWidth,borderColor:C.goldDim,backgroundColor:'rgba(7,12,23,0.72)',marginBottom:9},badgeTxt:{color:C.gold,fontSize:7.5,fontWeight:'800',letterSpacing:0.8},avatar:{width:48,height:48,borderRadius:24,borderWidth:2,borderColor:C.blueBorder}});
  ...
  return(
    <View style={{marginBottom:16}}>
      <View style={{flexDirection:'row',alignItems:'center',gap:6,paddingHorizontal:EDGE,marginBottom:11}}>
        ...
        <TouchableOpacity onPress={onIndustry} hitSlop={8} style={{marginLeft:'auto' as any,flexDirection:'row',alignItems:'center',gap:4,paddingHorizontal:9,paddingVertical:4,borderRadius:9,borderWidth:StyleSheet.hairlineWidth,borderColor:C.blueBorder,backgroundColor:C.blueFaint}}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{paddingHorizontal:EDGE,gap:9}}>
        {loading?[0,1,2,3].map(i=><View key={i} style={{width:122,height:152,borderRadius:13,backgroundColor:C.navyMid}}/>):
        pros.map(p=>(
...
export default function SocialScreen(){
  ...
  const onRefresh=useCallback(()=>{setRefreshing(true);setFilterTag(null);refresh();setTimeout(()=>setRefreshing(false),900);},[refresh]);
  const handleTag=useCallback((t:string)=>{
    setFilterTag(p=>p===t?null:t);
    if(Platform.OS!=='web') Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light).catch(()=>{});
    listRef.current?.scrollToOffset({offset:0,animated:true});
  },[]);
  const handleAddComment=useCallback((id:string,text:string)=>{addComment(id,text);},[addComment]);

  const renderItem=useCallback(({item}:{item:Critique})=>(
    <CritiqueCard
      item={item} userId={userId} critique={item}
      onLike={toggleLike} onComments={toggleComments}
      onTag={handleTag} onShare={shareCritique}
      onShareComment={shareComment}
      onAddComment={handleAddComment}
    />
  ),[userId,toggleLike,toggleComments,handleTag,shareCritique,shareComment,handleAddComment]);
    ...
    <View>
      ...
      <View style={sc.header}>
        ...
        <View style={{flexDirection:'row',gap:7,alignItems:'center'}}>
          ...
          <TouchableOpacity style={[sc.iconBtn,{borderColor:'rgba(90,150,230,0.35)',backgroundColor:'rgba(90,150,230,0.10)'}]} onPress={()=>setShowIndustry(true)} activeOpacity={0.85}>
            ...
            <View style={{position:'absolute',top:7,right:7,width:5,height:5,borderRadius:2.5,backgroundColor:C.blue}}/>
          </TouchableOpacity>
      ...
      <View style={sc.tabs}>
        {FEED_TABS.map(t=>{const on=t===tab;return(
          <TouchableOpacity key={t} onPress={()=>{setTab(t);setFilterTag(null);listRef.current?.scrollToOffset({offset:0,animated:true});}} style={sc.tab} activeOpacity={0.80}>
      ...
      <View style={{flexDirection:'row',alignItems:'center',gap:6,paddingHorizontal:EDGE,marginBottom:11}}>
        ...
        <Text style={{color:C.offWhite,fontSize:14,fontWeight:'800'}}>{tab==='Tendances'?'Critiques les plus appréciées':'Critiques récentes'}</Text>
        <View style={{paddingHorizontal:6,paddingVertical:1.5,borderRadius:6,backgroundColor:C.faint,borderWidth:StyleSheet.hairlineWidth,borderColor:C.border}}><Text style={{color:C.muted,fontSize:8,fontWeight:'700'}}>{displayed.length}</Text></View>
      </View>
    </View>
  );
    ...
    <View style={sc.root}>
      ...
      <SafeAreaView style={{flex:1}} edges={['top']}>
        <FlatList
          ...
          ListEmptyComponent={
            loading
              ?<View style={{alignItems:'center',paddingVertical:55,gap:12}}><ActivityIndicator color={C.mid} size="large"/><Text style={{color:C.muted,fontSize:12}}>Chargement…</Text></View>
              :<View style={{alignItems:'center',paddingVertical:55,gap:10}}>
                <View style={{width:54,height:54,borderRadius:27,backgroundColor:C.faint,alignItems:'center',justifyContent:'center',borderWidth:StyleSheet.hairlineWidth,borderColor:C.border}}><Ionicons name="create-outline" size={25} color={C.muted}/></View>
                <Text style={{color:C.muted,fontSize:13,fontWeight:'700'}}>Aucune critique</Text>
                <Text style={{color:C.muted,fontSize:11,textAlign:'center',paddingHorizontal:40,lineHeight:16}}>{filterTag?`Aucune critique pour #${filterTag}`:'Soyez le premier !'}</Text>
                {filterTag&&<TouchableOpacity onPress={()=>setFilterTag(null)} style={{paddingHorizontal:14,paddingVertical:7,borderRadius:11,borderWidth:StyleSheet.hairlineWidth,borderColor:C.blueBorder,backgroundColor:C.blueFaint}}><Text style={{color:C.blue,fontSize:12,fontWeight:'700'}}>Toutes les critiques</Text></TouchableOpacity>}
              </View>
          }
...
const sc=StyleSheet.create({
  ...
  lvlBtn:   {flexDirection:'row',alignItems:'center',paddingHorizontal:8,paddingVertical:5,borderRadius:9,borderWidth:StyleSheet.hairlineWidth,borderColor:C.blueBorder,backgroundColor:C.blueFaint},
