...
const GalaxyCanvas = memo(() => (
  <View style={StyleSheet.absoluteFill} pointerEvents="none">
    ...
    {/* L1 — bande Voie Lactée diagonale */}
    <View style={{ ...StyleSheet.absoluteFillObject, alignItems:'center', justifyContent:'center', overflow:'hidden' }}>
      <LinearGradient colors={[G.mwEdge,G.mwCore,G.mwEdge]} start={{x:0,y:0.5}} end={{x:1,y:0.5}}
        style={{ position:'absolute', width:W*3, height:H*0.72, transform:[{rotate:'-34deg'}] }} />
      <LinearGradient colors={[G.mwEdge,'rgba(138,62,228,0.082)',G.mwEdge]} start={{x:0,y:0.5}} end={{x:1,y:0.5}}
        style={{ position:'absolute', width:W*3, height:H*0.28, transform:[{rotate:'-34deg'}] }} />
    </View>
    {/* L2 — nébuleuses */}
    ...
    {/* L6 — croix ✦ */}
    {SPARKLES.map(sp=><SparkleCross key={`sp${sp.id}`} sp={sp} />)}
    {/* L7 — étoiles filantes */}
    <MeteorManager />
  </View>
));
...
export default function SearchScreen() {
  ...
  return (
    <View style={sc.root}>
      ...
      {/* ── Galaxy TabBar ───────────────────────────────────────── */}
      <GalaxyTabBar active={actTab} set={setActTab} />
    </View>
  );
}
