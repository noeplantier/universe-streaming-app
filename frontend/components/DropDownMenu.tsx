 ...
 * `key` de chaque item = `genres.value` EXACT nettoyé (`trim`) uniquement.
 * C'est cette valeur qui doit matcher la colonne `reels.genre` en DB.
 *
 * Parent (feed) :
 *   const handleGenreSelect = (key: MenuKey) => setActiveGenre(key);
...
const PANEL_W = Math.min(W * 0.8, 320);

// Helpers
function sanitizeGenreKey(raw: string | null | undefined): string {
  return (raw ?? '').trim();
}

// Affichage uniquement (ne sert PAS au filtre DB)
...
const PanelShoot = memo(function PanelShoot({ m, onDone }: { m: Met; onDone: () => void }) {
  ...
  return (
    <Animated.View style={{ position: 'absolute', left: m.sx, top: m.sy, opacity: op, transform: [{ translateX: tx }, { translateY: ty }, { rotate: `${m.ang}deg` }] }}>
      <LinearGradient
        colors={['transparent', 'rgba(255,255,255,0.15)', '#fff']}
        start={{ x: 0, y: 0 }}
        end={{ x: 1, y: 0 }}
        style={{ width: m.len, height: 1.2, borderRadius: 1 }}
      />
    </Animated.View>
  );
});
...
const PanelGalaxy = memo(function PanelGalaxy() {
  ...
  return (
    <View style={StyleSheet.absoluteFill} pointerEvents="none">
      ...
      {meteors.map((m) => (
        <PanelShoot key={m.id} m={m} onDone={() => setMeteors((ms) => ms.filter((x) => x.id !== m.id))} />
      ))}
      ...
      <View style={ph.profileRow}>
        ...
        <View style={{ flex: 1, gap: 2 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
            ...
            {profile.is_pro && (
              <View style={ph.proChip}>
                <Text style={ph.proChipTxt}>PRO</Text>
              </View>
            )}
          </View>
...
const ph = StyleSheet.create({
  ...
  proChip: {
    ...
    backgroundColor: 'rgba(245,200,66,0.20)', borderWidth: 1, borderColor: 'rgba(245,200,66,0.40)',
  },
...
const DropdownMenu = memo(function DropdownMenu({
    ...
    getDeviceId().then((uid) => {
      ...
      rtRef.current = supabase
        ...
        .on(
          ...
          ({ new: row }) => {
            if (alive) setProfile((prev) => (prev ? { ...prev, ...(row as any) } : prev));
          },
        )
        .subscribe();
    });
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!error && data) {
        ...
        const items: MenuItem[] = (data as GenreRow[])
          .map((g) => {
            const key = sanitizeGenreKey(g.value); // <-- clé exacte pour reels.genre
            const display = (g.label ?? '').trim() || toDisplayLabel(g.value); // <-- affichage propre
            return {
              key,
              label: display,
              icon: iconForGenre(key),
            };
          })
          .filter((x) => {
  ...
  useEffect(() => {
    if (visible) {
      ...
      Animated.parallel([
        ...
        Animated.stagger(
          10,
          itemAnims.map((a) => Animated.timing(a, { toValue: 1, duration: 140, useNativeDriver: true })),
        ),
      ]).start();
    } else {
  ...
  const handleSelect = useCallback((key: MenuKey) => {
    const safe = sanitizeGenreKey(key);
    hapticLight();
    onSelect(safe || ALL_GENRES_KEY);
    onClose();
  }, [onClose, onSelect]);
            ...
            {genreLoading
              ? Array.from({ length: 4 }).map((_, i) => (
                  <View key={i} style={{ paddingHorizontal: 16, paddingVertical: 8, flexDirection: 'row', gap: 12, alignItems: 'center' }}>
                    <Shimmer w={32} h={32} r={10} />
                    <Shimmer w={100} h={13} r={6} />
                  </View>
                ))
