...
const PostCard = memo(function PostCard({
  ...
  const bounce = useCallback((anim: Animated.Value) => {
    Animated.sequence([
      Animated.spring(anim, {
        toValue: 1.42, useNativeDriver: true, tension: 300, friction: 7,
      }),
      Animated.spring(anim, {
  ...
  // ★ Navigation corrigée ★
  // post.id     = UUID de la critique → ne pas utiliser pour naviguer vers /film/
  // post.work_id = bigint FK vers public.works → correct pour /film/[id]
  const goToFilm = useCallback(() => {
    if (!post.work_id) return; // pas de work lié → ne rien faire
    router.push(`/film/${post.work_id}` as any);
  }, [post.work_id, router]);

  // ── Mémoïsés ───────────────────────────────────────────────────────────────
  const toneInfo = useMemo(
    () => TONES.find(t => t.key === post.tone) ?? TONES[0],
    [post.tone],
  );
  ...
  return (
    <View style={s.card}>
      ...
      {/* ── Hero image ────────────────────────────────────────────────────── */}
      {/* ★ FIX 1/3 : post.id → post.work_id ★ */}
      <TouchableOpacity activeOpacity={0.92} onPress={goToFilm}>
        ...
        <View style={s.filmOverlay}>
          <Text style={s.filmTitle} numberOfLines={1}>
            {post.work_title || 'Œuvre inconnue'}
          </Text>
          {metaStr.length > 0 && (
            <Text style={s.filmMeta}>{metaStr}</Text>
          )}
      ...
      <View style={s.body}>
        ...
        <View style={s.authorRow}>
          ...
          {post.work_genre.length > 0 && (
            <View style={s.genrePill}>
              <Text style={s.genrePillTxt}>{post.work_genre}</Text>
            </View>
          )}
        </View>
        ...
        <View style={s.actions}>
          ...
          <TouchableOpacity style={s.action} onPress={onLike} activeOpacity={0.78}>
            ...
            <Text style={[s.actionTxt, isLiked && { color: C.red }]}>
          ...
          {/* Flèche ★ FIX 3/3 ★ */}
          <TouchableOpacity style={s.arrowBtn} onPress={goToFilm}>
